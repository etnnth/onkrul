module Index.Sort exposing (sortBySearch)
...
score search metadata = 
  let 
    distanceToTitle = min 0 ((distance search metadata.title) - 5) 
    distanceToKeyword keyword =
      case List.minimum (List.map (distance keyword) (String.words search)) of
        Nothing -> 0
        Just d -> min 0 (d-5)
    distanceToKeywords = List.map distanceToKeyword metadata.keywords
  in
